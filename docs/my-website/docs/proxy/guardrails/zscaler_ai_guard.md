...
Zscaler AI guard enforces security policies for all traffic towards AI
sites, models and applications. The AI guard is part of the Zero Trust
Exchange and provides a comprehensive platform for visibility, control
and deep packet inspection of AI prompts.
...
Expect this to fail since `ishaan@berri.ai` in the request is PII (Ensure that a PII detector within Zscaler AI Guard is enabled prior to processing this request)
...
In cases where Zscaler AI Guard encounters operational issues, it
returns: 
- **HTTP Status**: 500 
...
``` json
{
   "error": {
      ...
      "reason": "Action field in response is None, expecting 'ALLOW' or'BLOCK'
   },
   "type": "None",
   "param": "None",
   "code": "500"
